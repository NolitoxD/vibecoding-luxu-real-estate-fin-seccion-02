...
export default function LoginForm() {
  ...
  return (
    <div className="font-display bg-[#EEF6F6] min-h-screen flex items-center justify-center p-4 antialiased text-[#19322F] relative overflow-hidden">
      {/* Background blurs exactly from the code.html original */}
          ...
          <div className="space-y-4">
            <form
              onSubmit={async (e) => {
                e.preventDefault();
                setIsLoadingGoogle(true);
                const { signInUser } = await import('@/lib/actions/auth');
            ...
            >
              ...
              >
                {isLoadingGoogle ? "Cargando..." : "Iniciar sesión"}
              </button>
            </form>
            ...
            <button
              ...
              disabled={isLoadingGoogle || isLoadingGithub}
              className="group w-full flex items-center justify-center gap-3 bg-white border border-gray-200 rounded-lg p-3.5 text-[#19322F] font-medium transition-all duration-300 hover:shadow-[0_10px_25px_-5px_rgba(25,50,47,0.1)] hover:-translate-y-0.5 relative overflow-hidden disabled:opacity-70 disabled:hover:translate-y-0 disabled:hover:shadow-none"
            >
              ...
              {isLoadingGoogle ? (
                <div className="w-5 h-5 relative z-10 border-2 border-[#006655] border-t-transparent rounded-full animate-spin"></div>
              ) : (
                <svg
